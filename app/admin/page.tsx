...
export default function AdminPage() {
  ...
  if (!authed) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--header)'}}>
      <div style={{background:'linear-gradient(165deg,#fffdf7,#fff1cf)',border:'1px solid #FFCD81',borderRadius:14,padding:'48px 44px',width:400,textAlign:'center',boxShadow:'var(--shadow-lg)'}}>
        ...
        <p style={{fontWeight:700,fontSize:'1.3rem',color:'#3A2A15',letterSpacing:'-0.02em',marginBottom:5}}>Console Admin</p>
        ...
        <input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&login()} placeholder="Mot de passe"
          style={{width:'100%',fontSize:'0.87rem',background:'rgba(255, 172, 70, 0.35)',border:`1px solid ${loginErr?'#DC2626':'rgba(255,255,255,0.1)'}`,borderRadius:8,padding:'11px 14px',color:'#3A2A15',outline:'none',marginBottom:10,caretColor:'var(--accent)'}} />
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      ...
      {/* Header */}
      <header style={{background:'var(--header)',backdropFilter:'blur(10px)',borderBottom:'1px solid var(--header-border)',position:'sticky',top:0,zIndex:50,boxShadow:'var(--shadow-sm)'}}>
        ...
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 32px',display:'flex',alignItems:'center',justifyContent:'space-between',height:60}}>
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <div>
                <p style={{fontWeight:800,fontSize:'0.88rem',color:'#3A2A15',letterSpacing:'-0.02em',lineHeight:1.15}}>Console Admin</p>
                <p style={{fontFamily:'var(--font-mono)',fontSize:'0.54rem',color:'#A7B6FF',letterSpacing:'0.08em'}}>WorkDash · MISMO</p>
              </div>
            </div>
            <span style={{width:1,height:18,background:'#FFDCA7'}} />
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <button onClick={save} disabled={saving||!unsaved}
              style={{fontSize:'0.76rem',fontWeight:600,background:(unsaved&&!saving)?'var(--accent)':'rgba(255, 172, 70, 0.35)',border:`1px solid ${(unsaved&&!saving)?'transparent':'rgba(255, 172, 70, 0.35)'}`,borderRadius:7,padding:'7px 16px',color:(unsaved&&!saving)?'white':'#A06A22',cursor:(unsaved&&!saving)?'pointer':'not-allowed',transition:'all 0.2s'}}>
              {saving?'Sauvegarde…':'Sauvegarder'}
            </button>
            <a href="/" target="_blank" style={{fontSize:'0.76rem',fontWeight:500,border:'1px solid rgba(255, 172, 70, 0.35)',borderRadius:7,padding:'7px 14px',color:'#D6DEFF',textDecoration:'none',display:'inline-block'}}>
              Dashboard ↗
            </a>
            <button onClick={logout} style={{fontSize:'0.76rem',fontWeight:500,background:'none',border:'1px solid rgba(255, 172, 70, 0.28)',borderRadius:7,padding:'7px 12px',color:'#A7B6FF',cursor:'pointer'}}>Déconnexion</button>
          </div>
        </div>
      </header>
      ...
      <main style={{maxWidth:1280,margin:'0 auto',padding:'28px 32px 56px'}}>
        ...
        {tab==='projects'&&(
          <div>
            ...
            {/* Add form */}
            {showAdd&&(
              <div style={{background:'linear-gradient(165deg, rgba(20,30,74,0.82), rgba(8,13,35,0.8))',border:'1px solid var(--border)',borderRadius:18,padding:'24px',marginBottom:16,boxShadow:'var(--shadow-sm)'}} className="slide-up">
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {sorted.map(p=>{
                ...
                return (
                  <div key={p.id} style={{background:'linear-gradient(165deg, rgba(20,30,74,0.82), rgba(8,13,35,0.8))',border:`1px solid ${isExp?sm.color+'55':'var(--border)'}`,borderRadius:16,overflow:'hidden',boxShadow:isExp?`var(--shadow-sm),0 0 0 3px ${sm.color}10`:'var(--shadow-xs)',transition:'all 0.2s'}}>
                    {/* Status stripe */}
          ...
          <div style={{maxWidth:820}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:7}}>
              {comments.map(c=>{
                ...
                return (
                  <div key={c.id} style={{display:'flex',gap:14,background:'linear-gradient(165deg, rgba(20,30,74,0.82), rgba(8,13,35,0.8))',border:`1px solid ${isM?'#C4C2F7':'#86EFAC'}`,borderRadius:16,padding:'14px 16px',alignItems:'flex-start',boxShadow:'var(--shadow-xs)'}}>
